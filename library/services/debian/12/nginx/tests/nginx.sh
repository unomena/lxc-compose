...
# Get container IP
CONTAINER_IP=$(lxc list nginx-debian-12 -f json | jq -r '.[0].state.network.eth0.addresses[] | select(.family=="inet").address')
...
if curl -s http://$CONTAINER_IP | grep -q "Welcome to nginx-debian-12"; then
...
if curl -sI http://$CONTAINER_IP | grep -q "Server: nginx-debian-12"; then
