...
# Get container IP
CONTAINER_IP=$(lxc list haproxy-debian-12 -f json | jq -r '.[0].state.network.eth0.addresses[] | select(.family=="inet").address')
...
if lxc exec haproxy-debian-12 -- pgrep haproxy-debian-12 > /dev/null; then
