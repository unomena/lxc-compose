...
# Get container IP
CONTAINER_IP=$(lxc list nginx-minimal-24-04 -f json | jq -r '.[0].state.network.eth0.addresses[] | select(.family=="inet").address')
...
if curl -s http://$CONTAINER_IP | grep -q "Welcome to nginx-minimal-24-04"; then
...
if curl -sI http://$CONTAINER_IP | grep -q "Server: nginx-minimal-24-04"; then
