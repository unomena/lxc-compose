...
CONTAINER_NAME=""
for name in postgresql-alpine-3-19 postgresql-alpine-3-19ql-server; do
...
if [ -z "$CONTAINER_NAME" ]; then
    echo -e "${RED}✗${NC} PostgreSQL container not found (tried: postgresql-alpine-3-19, postgresql-alpine-3-19ql-server)"
    exit 1
fi
...
lxc exec $CONTAINER_NAME -- su postgresql-alpine-3-19 -c "createdb testdb"
if [ $? -eq 0 ]; then
...
lxc exec $CONTAINER_NAME -- su postgresql-alpine-3-19 -c "psql -d testdb -c 'CREATE TABLE test_table (id SERIAL PRIMARY KEY, name VARCHAR(50), created_at TIMESTAMP DEFAULT NOW());'"
if [ $? -eq 0 ]; then
...
lxc exec $CONTAINER_NAME -- su postgresql-alpine-3-19 -c "psql -d testdb -c \"INSERT INTO test_table (name) VALUES ('Test Record');\""
if [ $? -eq 0 ]; then
...
RESULT=$(lxc exec $CONTAINER_NAME -- su postgresql-alpine-3-19 -c "psql -d testdb -t -c \"SELECT name FROM test_table WHERE name='Test Record';\"" | tr -d ' ')
...
lxc exec $CONTAINER_NAME -- su postgresql-alpine-3-19 -c "psql -d testdb -c \"DELETE FROM test_table WHERE name='Test Record';\""
if [ $? -eq 0 ]; then
...
lxc exec $CONTAINER_NAME -- su postgresql-alpine-3-19 -c "psql -d testdb -c 'DROP TABLE test_table;'"
if [ $? -eq 0 ]; then
...
lxc exec $CONTAINER_NAME -- su postgresql-alpine-3-19 -c "dropdb testdb"
if [ $? -eq 0 ]; then
