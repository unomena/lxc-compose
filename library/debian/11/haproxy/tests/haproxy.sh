...
# Get container IP
CONTAINER_IP=$(lxc list haproxy-debian-11 -f json | jq -r '.[0].state.network.eth0.addresses[] | select(.family=="inet").address')
...
if lxc exec haproxy-debian-11 -- pgrep haproxy-debian-11 > /dev/null; then
