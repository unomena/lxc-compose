...
# Get container IP
CONTAINER_IP=$(lxc list haproxy-minimal-22-04 -f json | jq -r '.[0].state.network.eth0.addresses[] | select(.family=="inet").address')
...
if lxc exec haproxy-minimal-22-04 -- pgrep haproxy-minimal-22-04 > /dev/null; then
