#!/bin/bash
# Port forwarding tests for Flask application
...
echo "=== Port Forwarding Tests for Flask Application ==="
echo
...
check_iptables_forward() {
    ...
    if [ -n "$DNAT_RULE" ]; then
        ...
        if lxc config show sample-flask-app 2>/dev/null | grep -q "proxy.*$port" || \
           lxc config device show sample-flask-app 2>/dev/null | grep -q "$port"; then
...
check_orphaned_rules() {
    ...
    for port in $RULES; do
        ...
        if [ "$container_name" == "sample-flask-app" ]; then
        ...
        if [ "$container_name" == "sample-flask-datastore" ]; then
...
check_orphaned_rules "$APP_IP" "sample-flask-app"
...
check_orphaned_rules "$DATASTORE_IP" "sample-flask-datastore"
