#!/bin/bash
# Port forwarding tests for Django + Celery application
...
echo "=== Port Forwarding Tests for Django + Celery ==="
echo
...
check_iptables_forward() {
    ...
    if [ -n "$DNAT_RULE" ]; then
        ...
        if lxc config show sample-django-app 2>/dev/null | grep -q "proxy.*$port" || \
           lxc config device show sample-django-app 2>/dev/null | grep -q "$port"; then
...
check_orphaned_rules() {
    ...
    for port in $RULES; do
        ...
        if [ "$container_name" == "sample-django-app" ]; then
        ...
        if [ "$container_name" == "sample-datastore" ]; then
